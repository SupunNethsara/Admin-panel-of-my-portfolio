...
import { FiBookOpen, FiUpload, FiCheck, FiAlertCircle, FiArrowLeft, FiSave } from 'react-icons/fi';
import { db } from '../../../firebaseConfig';
import { uploadImage } from '../Cloudinaryapi';

function LicenseCertificateEditForm() {
  const { id } = useParams();
  ...
  const [formData, setFormData] = useState({
    ...
    icon: '', issuerIcon: '',
    newCertFile: null, newCertPreview: '',
    newIssuerIconFile: null, newIssuerIconPreview: '',
  });
  ...
  useEffect(() => {
    const fetchCert = async () => {
      try {
        const docSnap = await getDoc(doc(db, 'Licensecertificates', id));
        ...
        console.error('Error fetching license certificate:', err);
        setNotFound(true);
      } finally {
        setLoading(false);
      }
  ...
  const handleCertFileChange = (e) => {
    ...
    if (!file.type.match('image.*') && !file.type.match('pdf')) {
      setErrorMessage('Only image/PDF files allowed for certificate file');
      return;
    }
    if (file.size > 2 * 1024 * 1024) { setErrorMessage('File must be under 2MB'); return; }
    setFormData(prev => ({ ...prev, newCertFile: file, newCertPreview: URL.createObjectURL(file) }));
    setErrorMessage('');
  };

  const handleIssuerIconChange = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    if (!file.type.match('image.*')) { setErrorMessage('Only image files allowed for issuer icon'); return; }
    if (file.size > 2 * 1024 * 1024) { setErrorMessage('Issuer icon must be under 2MB'); return; }
    setFormData(prev => ({ ...prev, newIssuerIconFile: file, newIssuerIconPreview: URL.createObjectURL(file) }));
    setErrorMessage('');
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let issuerIconUrl = formData.issuerIcon;

      if (formData.newCertFile) {
        const res = await uploadImage(formData.newCertFile, 'portfolio_certs', (p) => setUploadProgress(p));
        iconUrl = res.data.secure_url;
      }
      if (formData.newIssuerIconFile) {
        const res = await uploadImage(formData.newIssuerIconFile, 'portfolio_certs', (p) => setUploadProgress(p));
        issuerIconUrl = res.data.secure_url;
      }

      await updateDoc(doc(db, 'Licensecertificates', id), {
        title: formData.title,
        issuer: formData.issuer,
        date: formData.date,
        icon: iconUrl,
        issuerIcon: issuerIconUrl,
        link: formData.link || '#',
      ...
      console.error('Error updating license certificate:', err);
    ...
    return (
      <div className="max-w-3xl">
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-16 text-center">
          ...
          <button onClick={() => navigate('/dashboard/licenseCertificate')} className="text-emerald-600 text-sm font-medium hover:underline">
            Back to License Certificates
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-800">Edit License Certificate</h1>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Certificate Name <span className="text-red-400">*</span></label>
              <input type="text" name="title" value={formData.title} onChange={handleChange} required className={inputClass} />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Institute Name <span className="text-red-400">*</span></label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Issue Date <span className="text-red-400">*</span></label>
            ...
            <div>
              ...
              <input type="text" name="credentialsId" value={formData.credentialsId} onChange={handleChange} className={inputClass} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Certificate File</label>
              <label className="flex items-center gap-3 px-4 py-3 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group h-[72px]">
                ...
                <div className="min-w-0">
                  <p className="text-sm text-slate-500 group-hover:text-indigo-600 truncate">
                    {formData.newCertFile ? formData.newCertFile.name : 'Replace file (optional)'}
                  </p>
                  <p className="text-xs text-slate-400">PDF, PNG, or JPG (Max 2MB)</p>
                </div>
                <input type="file" accept="application/pdf,image/*" onChange={handleCertFileChange} className="hidden" />
              </label>
              {(formData.newCertPreview || formData.icon) && (
                <div className="mt-2 w-full h-24 rounded-xl overflow-hidden border border-slate-200">
                  <img src={formData.newCertPreview || formData.icon} alt="Certificate preview" className="w-full h-full object-cover" />
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <label className="flex-1 flex items-center gap-3 px-4 py-3 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group h-[72px]">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm text-slate-500 group-hover:text-indigo-600 truncate">
                      {formData.newIssuerIconFile ? formData.newIssuerIconFile.name : 'Replace icon (optional)'}
                    </p>
                    <p className="text-xs text-slate-400">PNG, JPG, SVG (Max 2MB)</p>
                  </div>
                  <input type="file" accept="image/*" onChange={handleIssuerIconChange} className="hidden" />
                </label>
                <div className="w-[72px] h-[72px] border border-slate-200 rounded-xl overflow-hidden shrink-0 bg-white shadow-sm flex items-center justify-center">
                  {(formData.newIssuerIconPreview || formData.issuerIcon) ? (
                    <img src={formData.newIssuerIconPreview || formData.issuerIcon} alt="Icon preview" className="w-full h-full object-contain p-1" />
                  ) : (
          ...
          {isSubmitting && uploadProgress > 0 && (
            <div>
              ...
              <div className="w-full bg-slate-100 rounded-full h-2">
                <div className="bg-gradient-to-r from-emerald-500 to-teal-500 h-2 rounded-full transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
              </div>
            </div>
          )}
          ...
          <div className="pt-1 flex items-center gap-3">
            ...
            <button type="button" onClick={() => navigate('/dashboard/licenseCertificate')}
...
export default LicenseCertificateEditForm;